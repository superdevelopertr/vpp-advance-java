...
public class BookCollection {
	...
	public Map<Integer, String> findBookMap(Predicate<Book> searchCriteria) {
		return findBooks(searchCriteria).stream()
				.collect(Collectors.toMap((Book a) -> a.getId(), (Book a) -> a.getTitle()));
	}
	
	public Long findBookSizeByCriteria(Predicate<Book> searchCriteria) {
		return findBooks(searchCriteria).stream().count();
	}
