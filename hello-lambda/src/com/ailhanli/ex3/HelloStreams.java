...
public class HelloStreams {
	...
	public static double findTotal(List<Double> items) {
		...
		return stream.filter(a -> a > 15).reduce(0d, (a, b) -> a + b);
	}

	public static List<Double> getDouble(List<Double> items) {
		Stream<Double> stream = items.stream();
		return stream.filter(a -> a > 15).map(a -> a * 2).collect(Collectors.toList());
	}
	...
	public static void main(String[] args) {
		...
		getDouble(items).forEach(a -> System.out.println(a));
